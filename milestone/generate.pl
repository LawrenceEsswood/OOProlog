...
class_def_class_token(T):-class_def__classType(X),build_class(X,T,[]),::(X,'Constructor',T6), (T6=none;T6\=none,call(T6,T,class_token,['Hdr','Bdy'],[],none,none,p_class_token_class_token)).
p_class_token_class_token(This,T7,T8):- ::(This,'Hdr',T7),::(This,'Bdy',T8).
class_def_clause_token(T):-class_def__classType(X),build_class(X,T,[]),::(X,'Constructor',T9), (T9=none;T9\=none,call(T9,T,clause_token,['Left','Name','N','Mods'],[ (split_fnc,p_clause_token_split_fnc), (check_mod_allowed,p_clause_token_check_mod_allowed)],none,none,none)).
...
class_def_rule_token(T):-class_def__classType(X),build_class(X,T,[]),::(X,'Constructor',T14), (T14=none;T14\=none,call(T14,T,rule_token,['Left','Name','N','Mods','Right'],[ (add_mod,p_rule_token_add_mod), (split_fnc,p_clause_token_split_fnc), (check_mod_allowed,p_clause_token_check_mod_allowed)],none,none,p_rule_token_rule_token)).
...
p_rule_token_add_mod(This,Mod,Tkn):- ::(This,check_mod_allowed,T23),call(T23,Mod),class_def_rule_token(T25),::(This,'Left',T26),::(This,'Right',T27),::(This,'Mods',T28),build_class(T25,Tkn,[]),::(T25,'Constructor',T24), (T24=none;T24\=none,call(T24,Tkn,T26,T27,[Mod|T28])).
class_def_fact_token(T):-class_def__classType(X),build_class(X,T,[]),::(X,'Constructor',T29), (T29=none;T29\=none,call(T29,T,fact_token,['Left','Name','N','Mods'],[ (add_mod,p_fact_token_add_mod), (split_fnc,p_clause_token_split_fnc), (check_mod_allowed,p_clause_token_check_mod_allowed)],none,none,p_fact_token_fact_token)).
...
p_fact_token_add_mod(This,Mod,Tkn):- ::(This,check_mod_allowed,T36),call(T36,Mod),class_def_fact_token(T38),::(This,'Left',T39),::(This,'Mods',T40),build_class(T38,Tkn,[]),::(T38,'Constructor',T37), (T37=none;T37\=none,call(T37,Tkn,T39,[Mod|T40])).
:-op(100,'yfx','::').
:-op(99,'fx','class').
:-op(100,'xfx','extends').
:-op(1199,'fy','static').
...
p_parser_parse_include(This, (:-oopl_include(Path)),Defs):-open(Path,'read',In),class_def_io(T53),::(This,'IO',T55),::(T55,'Out',T54),build_class(T53,NewIO,[]),::(T53,'Constructor',T52), (T52=none;T52\=none,call(T52,NewIO,In,T54)),class_def_parser(T57),build_class(T57,P,[]),::(T57,'Constructor',T56), (T56=none;T56\=none,call(T56,P,NewIO)),::(P,'parse_defs',T58),call(T58,Defs,'end_of_file'),close(In).
p_parser_parse_field(_,X,field(X)):-'!'.
p_parser_parse_class(This,T,class_def(C)):- ::(This,parse_header,T59),call(T59,T,H),::(This,parse_defs,T60),call(T60,Defs,'endclass'),class_def_class_token(T62),build_class(T62,C,[]),::(T62,'Constructor',T61), (T61=none;T61\=none,call(T61,C,H,Defs)).
p_parser_parse_header(_,class(X)extends Y,H):-atomic(X),atomic(Y),'!',class_def_class_header_token(T64),build_class(T64,H,[]),::(T64,'Constructor',T63), (T63=none;T63\=none,call(T63,H,X,[Y])).
p_parser_parse_header(_,class(X),H):-atomic(X),class_def_class_header_token(T66),build_class(T66,H,[]),::(T66,'Constructor',T65), (T65=none;T65\=none,call(T65,H,X,[])).
...
p_parser_parse_clause(This,Eqs, (:-Xs),rule(R)):-'!',::(This,tag_atoms,T72),call(T72,Eqs,Xs,Ys),class_def_rule_token(T74),build_class(T74,R,[]),::(T74,'Constructor',T73), (T73=none;T73\=none,call(T73,R,'',Ys)).
p_parser_parse_clause(This,Eqs, (P:-Xs),rule(R)):-'!',::(This,tag_predicate,T75),call(T75,Eqs,P,Pd),::(This,tag_atoms,T76),call(T76,Eqs,Xs,Ys),class_def_rule_token(T78),build_class(T78,R,[]),::(T78,'Constructor',T77), (T77=none;T77\=none,call(T77,R,Pd,Ys)).
p_parser_parse_clause(This,Eqs,P,fact(F)):- ::(This,tag_predicate,T79),call(T79,Eqs,P,Pd),class_def_fact_token(T81),build_class(T81,F,[]),::(T81,'Constructor',T80), (T80=none;T80\=none,call(T80,F,Pd)).
...
p_compile(In,Out,Extras,Opts):-class_def_generator(T175),build_class(T175,G,[]),::(T175,'Constructor',T174), (T174=none;T174\=none,call(T174,G,Opts,In,Out,Extras)),::(G,'g_compile',T176),call(T176).
...
p_generator_make_predicate_name(This,pred(Nm,_),_,Nm):- ::(This,'Opts',T227),member('no_munge',T227),'!'.
p_generator_make_predicate_name(_,pred('',_),_,'').
p_generator_make_predicate_name(This,pred(PredicateName,_),Scope,Result):-PredicateName\='',::(Scope,'Prefix',T228),::(This,append_if_not_blank,T229),call(T229,T228,PredicateName,Result).
p_generator_make_class_type_functor(_,ClassName,Scope,Result):- ::(Scope,'Prefix',T230),atomic_list_concat(['class_def',T230,ClassName],Result).
p_generator_make_class_type_args(This,Scope,Bdy):- ::(This,add_this,T231),call(T231,['T'],Scope,Bdy).
p_generator_make_class_type_body(This,Name,Fields,Predicates,Bdy,Constructor,Statics):- ::(This,'g_add_qoutes',T232),maplist(T232,Fields,QFields),::(This,next_tmp,T233),call(T233,T),::(This,expand_new,T234),call(T234,new('X','T',Name,QFields,Predicates,'none','none',Constructor),T,Statics,NewComs),list_to_comma_functor([class_def__classType('X')|NewComs],Bdy).
p_generator_expand_new(This,Command,Con,Result):- ::(This,expand_new,T235),call(T235,Command,Con,[],Result).
